    ...
    return (req, res, next) => {
      ...
      try {
        requiredFields.forEach(field => {
          ...
          switch (field.type) {
            case 'boolean':
              newData[field.name] = MiddlewareHandler.validateBoolean(value);
              break;
            case 'int':
              newData[field.name] = MiddlewareHandler.validateInt(value);
              break;
            case 'float':
              newData[field.name] = MiddlewareHandler.validateFloat(value);
              break;
            case 'array':
              if (!Array.isArray(value)) {
                throw new Error(`Invalid array value for field ${field.name}`);
              }
              //newData[field.name] = value; // Podrías agregar validaciones extra aquí
              break;
            case 'string':
            default:
    ...
    try {
      ...
      try {
        requiredFields.forEach(field => {
          ...
          switch (field.type) {
            case 'boolean':
              firstData[field.name] = MiddlewareHandler.validateBoolean(value);
              break;
            case 'int':
              firstData[field.name] = MiddlewareHandler.validateInt(value);
              break;
            case 'float':
              firstData[field.name] = MiddlewareHandler.validateFloat(value);
              break;
            case 'array':
              if (!Array.isArray(value)) {
                throw new Error(`Invalid array value for field ${field.name}`);
      ...
      const validatedSecondData = secondData.map((item, index) => {
        ...
        secondFields.forEach(field => {
          ...
          switch (field.type) {
            case 'boolean':
              item[field.name] = MiddlewareHandler.validateBoolean(value);
              break;
            case 'int':
              item[field.name] = MiddlewareHandler.validateInt(value);
              break;
            case 'float':
              item[field.name] = MiddlewareHandler.validateFloat(value);
              break;
            case 'array':
              if (!Array.isArray(value)) {
                throw new Error(`Invalid array value for field ${field.name}`);
    ...
    try {
      requiredFields.forEach(field => {
        ...
        if (value === undefined) {
          ...
          switch (field.type) {
            case 'boolean':
              value = MiddlewareHandler.validateBoolean(value);
              break;
            case 'int':
              value = MiddlewareHandler.validateInt(value);
              break;
            case 'float':
              value = MiddlewareHandler.validateFloat(value);
              break;
            case 'string':
            default:
              if (typeof value !== 'string') {
