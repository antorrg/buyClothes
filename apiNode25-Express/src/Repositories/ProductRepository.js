...
class ProductRepository extends BaseRepository{
    ...
    async getProduct(page = 1, size, name = null, trademark = null, fields = null, isAdmin= false, ) {
    ...
    async specialUpdate(id, newData) {
        ...
        try {
            ...
            for (const { value, type } of attributesToUpdate) {
                if (value) {
                    const attribute = await this.ModelAttributes.findByPk(value, { transaction: t });
                    if (!attribute) throwError(`This ${type.toLowerCase()} doesn't exist`, 404);
                    await product.addAttribute(attribute, { through: { type }, transaction: t });
                }
            }
